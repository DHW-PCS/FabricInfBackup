        ...
        switch(entrypoint.config.rollback_method) {
            case SHUTDOWN -> {
                if (!entrypoint.selectedBackups.add(info)) {
                    source.sendFeedback(new LiteralText("The same backup is already requested to be applied").formatted(Formatting.RED), false);
                    return 1;
                }
                source.sendFeedback(new LiteralText("The following backup is chosen").formatted(Formatting.AQUA), false);
                source.sendFeedback(Text.of(info.getRight().toString()), false);
                source.sendFeedback(new LiteralText("When the server shuts down, the rollback will be performed.").formatted(Formatting.GREEN), false);
            }
            case INSTANT -> {
                entrypoint.operationToConfirm.put(ctx.getSource().getName(), new LastOperation.RollbackDirect(id, entrypoint.storage));
                source.sendFeedback(new LiteralText("Rollback save in slot " + id + ", are you sure?").formatted(Formatting.AQUA), false);
